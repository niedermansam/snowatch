...
function SnotelCard({
  ...
  return (
    <div
      className={twMerge(
        " flex min-h-48 w-full    flex-col  items-center rounded p-4 text-center text-xl font-bold shadow ",
        className
      )}
    >
      {children}
...
function LoadingCard({
  ...
  return (
    <SnotelCard className="opacity-70 md:w-[400px]">
...
export function SnotelSection({ geohash }: { geohash: string }) {
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      <h3 className="text-xl font-bold">Nearby Snotel</h3>
      <div className="3xl:grid-cols-3 grid w-full justify-items-stretch gap-4 xl:grid-cols-2">
        {snotel.data.map((x) => {
          return (
            <SnotelSummary
...
function SnotelSummary({ snotel }: { snotel: NearbySnotel }) {
  ...
  return (
    <SnotelCard>
      ...
      <Tabs defaultValue="snow" className="w-full md:w-[400px]">
        ...
        <TabsContent value="snow" className="w-full">
...
function SnowAndSweSection({ data }: { data: NearbySnotel["data"]["data"] }) {
  if (!data) return null;
  ...
  return (
    <div className="grid  w-full flex-col items-stretch justify-stretch gap-x-6 gap-y-4 md:grid-cols-2  lg:flex-row">
...
function SnowDepthSection({ data }: { data: NearbySnotel["data"]["data"] }) {
  if (!data) return null;
  return (
    <div className="grid w-full grid-cols-3 justify-between gap-x-2 gap-y-1 text-center text-sm md:w-48 ">
      ...
      <div className="border-r border-sw-gray-200 pr-2 font-light">
...
function SweSection({ data }: { data: NearbySnotel["data"]["data"] }) {
  ...
  return (
    <div className="grid w-full grid-cols-3 gap-x-2 text-center text-sm md:w-48">
