...
class Geohash {
  ...
  static decode(geohash: string) {
    ...
    // cell centre
    let lat: string | number = (latMin + latMax) / 2;
    let lon: string | number = (lonMin + lonMax) / 2;
   ...
   * @throws  Invalid geohash.
   */
  static adjacent(geohash:string, direction:  Direction) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/prefer-includes
